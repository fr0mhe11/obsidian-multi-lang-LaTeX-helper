...
export default class Fcitx5LatexPlugin extends Plugin {
    ...
    async onload() {
        ...
        console.log("🚀 LaTeX 듀얼 엔진 플러그인 로드됨 (첫 글자 한글 누수 완벽 패치)");
    ...
    // ✨ 핵심: Range 슬라이스를 적용한 하이브리드 엔진 (첫 글자 누수 패치됨)
    private checkMathEnvironment(state: EditorState, pos: number): boolean {
        ...
        // 2. [판단 B] 설정된 Range만큼 잘라와서 State Machine 탐색 (블록과 인라인의 줄바꿈 로직 분리)
        ...
        for (let i = 0; i < textToScan.length; i++) {
            ...
            if (char === '$' && !isEscaped) {
                ...
                // ✨ 핵심 수정: 줄바꿈을 만났을 때 블록 수학($$)은 절대 닫히지 않습니다.
