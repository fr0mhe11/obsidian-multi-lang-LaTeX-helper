...
export default class Fcitx5LatexPlugin extends Plugin {
	...
	private lastPos = -1; // ✨ 최적화: 이전 커서 위치 기억 변수 추가

	async onload() {
		await this.loadSettings();
		console.log("🚀 LaTeX 듀얼 엔진 플러그인 로드됨 (트리거 필터링 & 락 최적화 완료)");
		...
		// ✨ 최적화 하이라이트: 무차별적 수식 검사 차단
		const updateListener = EditorView.updateListener.of((update) => {
			const currentPos = update.state.selection.main.head;
			
			// 실제로 글자가 입력/삭제되었거나(docChanged), 커서의 위치가 진짜로 이동했을 때만 실행
			if (update.docChanged || (update.selectionSet && currentPos !== this.lastPos)) {
	...
	private checkMathEnvironment(state: EditorState, pos: number): boolean {
		...
		for (let i = 0; i < textToScan.length; i++) {
			...
			if (char === '$' && !isEscaped) {
				...
				if (!blockMathOpen && inlineDollarOpen) {
